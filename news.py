...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             WELLCOME : [green]JAHIDUL PUBLIC TOOL[purple] '))
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold ]           • CHOOSE CLONING MENU • [/bold ]'))
	print('')
...
def dump_massal():
	...
	try:
		print('')
		print(f'{k}=>TOTAL CRACKING ID★{b}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
		...
		print(f'➥=>{k} IF ID IS PUBLIC THEN TRY AGAIN WITH NEW COOKIE OTHRWSE CHECK YOUR ID LNK {u}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
#-------------[ Id -Crackiing]---------------#
def setting():
	cetak(nel('\t[bold ]         CLONING MENU • [/bold ]'))
	print('') 
	...
	if hu in ['1','01']:
		...
		print('=>TRY AGAIN')
		exit()
	cetak(nel('\t[bold ]             • METHODE LOGIN • [/bold ]'))
	print('') 
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold ]               WAIT FOR A WHILE 😂[bold ]')) 
	print(f'                    {m}J {k}A {h}H {u}I {b}D {u}  ★  {b}K {u}I {h} N{k} G{m} OK{b}')
	print('')
